...
export default function MissionControl() {
  const [team, setTeam] = useState<Agent[]>(mockTeam);
  ...
  const handleApproval = (id: string, action: "approved" | "denied") => {
    ...
    // If approved, append system reaction
      ...
      setTimeout(() => {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            content: "Permission granted. I have read the file. The original testimonial markup is loaded into our Code workspace tab. I am now drafting the new HSL linear-gradient border wrapper. You can review the proposed diff in the Code tab.",
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }
        ]);
        setActiveTab("code");
    ...
    <div className="flex flex-col h-screen w-full bg-[#060810] text-[#e6edf3] font-sans antialiased overflow-hidden selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      <header className="h-12 w-full bg-[#11161d] border-b border-[#1e2632] flex items-center px-4 justify-between shrink-0 select-none">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-2.5 py-1 bg-[#161c25] border border-[#1e2632] rounded-md cursor-pointer hover:bg-[#1c2330] transition-colors">
            ...
            <span className="text-xs font-semibold text-[#e6edf3]">AXOD CREATIVE</span>
        ...
        <section className="w-[280px] bg-[#11161d] border-r border-[#1e2632] flex flex-col shrink-0">
          ...
          <div className="p-4 border-b border-[#1e2632] bg-gradient-to-b from-[#00e0ff]/[0.03] to-transparent relative group">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-width-0">
                <div className="font-semibold text-sm text-[#e6edf3] font-heading leading-tight">Sage</div>
            ...
            <div className="mt-3 p-2 bg-[#161c25] border border-[#1e2632] rounded text-[11px] leading-relaxed text-[#8b949e]">
              ...
              {isTyping ? (
                ...
                <span className="text-[#e6edf3] line-clamp-2">Sage is orchestrating Atlas on dynamic borders logic.</span>
              )}
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            <div className="p-1.5 flex flex-col gap-1">
              {team.filter(a => a.id !== 'sage').map((member) => (
                <div 
                  key={member.id}
                ...
                >
                  <div className="flex items-start gap-2.5">
                    <div className={`w-8 h-8 rounded-md bg-gradient-to-br ${member.color} flex items-center justify-center font-semibold text-black relative`}>
                      {member.avatar}
                      <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[#11161d] ${
                  ...
                  <div className="flex justify-between items-center text-[9px] font-mono text-[#5c6470]">
                    <span className="bg-[#1c2330] text-[#8b949e] px-1.5 py-0.5 rounded border border-[#2a3441]">{member.model}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="flex-1 bg-[#0a0e14] border-r border-[#1e2632] flex flex-col min-w-[400px]">
          ...
          <div className="h-11 w-full bg-[#11161d] border-b border-[#1e2632] px-4 flex items-center gap-2 justify-between shrink-0 select-none">
            ...
            <div className="text-[10px] font-mono text-[#8b949e] flex items-center gap-1">
              ...
              <code className="bg-[#161c25] border border-[#1e2632] px-1.5 py-0.2 rounded text-[#00e0ff]">c:/Users/A'KeemDrew/AXOD/landing</code>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((msg) => (
              <div key={msg.id} className="max-w-full">
                ...
                {msg.role !== 'system' && (
                  <div className={`text-xs leading-relaxed p-3 rounded-md border ${
                    msg.role === 'user' 
                    ...
                    {msg.dispatch && (
                      <div className="mt-3 p-2.5 bg-[#060810] border border-cyan-500/10 rounded-md relative group overflow-hidden">
                        ...
                        <div className="flex items-center gap-2 text-xs font-semibold text-[#e6edf3]">
                          <span className="w-5 h-5 rounded bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center text-[10px] text-black">⚒</span>
                          Atlas <ArrowRight className="w-3 h-3 text-[#5c6470]" /> Lead Developer
                        </div>
                ...
                {/* Tool Permission Approval Interactive Card */}
                {msg.approval && (
                  <div className="mt-2 p-3.5 bg-[#d29922]/[0.05] border border-[#d29922]/40 rounded-lg shadow-lg relative overflow-hidden">
                    ...
                    <div className="text-xs text-[#e6edf3] mb-3 leading-relaxed">
                      Atlas is requesting authorization to invoke <code className="bg-[#11161d] border border-[#1e2632] px-1.5 py-0.5 rounded text-cyan-400 font-mono text-[10.5px]">{msg.approval.toolName}</code>:
          ...
          <div className="h-11 bg-[#11161d] border-b border-[#1e2632] flex items-center justify-between px-4 shrink-0 select-none">
            <div className="flex h-full gap-1">
              ...
              >
                ...
                Code Diff
                <span className="bg-cyan-500/10 border border-cyan-500/25 text-[#00e0ff] text-[8.5px] px-1 py-0.2 rounded font-bold">1</span>
              </button>
              ...
              <ScrollArea className="h-full bg-[#11161d] border border-[#1e2632] rounded-lg p-5">
                <div className="prose prose-invert max-w-none text-xs text-[#8b949e] font-mono">
                  <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#2a3441]">
                    <h2 className="text-sm font-bold text-[#e6edf3] font-heading uppercase tracking-wide">Dynamic Plan</h2>
                    <span className="bg-[#161c25] border border-[#2a3441] px-2 py-0.5 rounded text-[10px] text-cyan-400">UPDATED</span>
                  </div>
            ...
            {activeTab === "code" && (
              <div className="h-full flex flex-col bg-[#11161d] border border-[#1e2632] rounded-lg overflow-hidden">
                <div className="h-9 w-full bg-[#161c25] border-b border-[#1e2632] px-4 flex items-center justify-between text-xs select-none">
                  ...
                  <span className="text-[9.5px] font-mono text-[#3fb950] bg-[#3fb950]/10 px-2 py-0.2 rounded border border-[#3fb950]/30">DIFF READY</span>
                </div>
                ...
                <ScrollArea className="flex-1 font-mono p-4 text-[11px] leading-relaxed bg-[#060810] overflow-x-auto">
                  <div className="whitespace-pre min-w-max text-[#8b949e]">
                    {artifacts.find(a => a.type === 'code')?.content.split('\n').map((line, idx) => {
                      const isDeleted = line.startsWith('-') || line.includes('<<<< ORIGINAL');
                      const isAdded = line.startsWith('+') || line.includes('====') || line.includes('>>>>') || line.includes('<!--');
                      let lineClass = "text-[#8b949e]";
                      let bgClass = "";
                      
                      if (line.includes('<<<< ORIGINAL') || line.includes('====') || line.includes('>>>>')) {
                        lineClass = "text-yellow-500 font-bold border-y border-yellow-500/20 block py-0.5 my-1";
            ...
            {activeTab === "terminal" && (
              <div className="h-full flex flex-col bg-black border border-[#1e2632] rounded-lg overflow-hidden font-mono shadow-inner shadow-black">
                <div className="h-9 w-full bg-[#11161d] border-b border-[#1e2632] px-4 flex items-center justify-between text-xs select-none">
                  ...
                  <span className="text-[9px] bg-[#161c25] px-1.5 py-0.5 rounded border border-[#2a3441] text-[#8b949e]">xterm-stream</span>
                </div>
                
                <ScrollArea className="flex-1 p-4 text-xs font-mono leading-relaxed bg-black text-[#8b949e]">
                  <pre className="whitespace-pre-wrap">
                    {artifacts.find(a => a.type === 'terminal')?.content}
                  </pre>
                  
                  {/* Dynamic interactive command prompt */}
      ...
      <footer className="h-8 w-full bg-[#11161d] border-t border-[#1e2632] px-4 flex items-center justify-between text-[10px] font-mono text-[#5c6470] shrink-0 select-none">
        <div className="flex items-center gap-4">
          ...
          <span>•</span>
          <span>WORKDIR: <code className="text-[#8b949e]">/srv/worktrees/sess_a4f9</code></span>
        </div>
        <div className="flex items-center gap-3">
