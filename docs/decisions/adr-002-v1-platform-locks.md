...
| 3 | **Production domain** | `mc-dev.axodcreative.pages.dev` (Cloudflare Pages preview style) for early access. Promote to `mc.axodcreative.com` after AXOD CREATIVE issue #16 (custom-domain DNS work) ships. | AXOD CREATIVE issue #16 closes. |
...
3. **`mc-dev.axodcreative.pages.dev`** — already-working preview domain, zero DNS work. Promote to apex when the rest of the domain story is ready.
